...
class TCBBoxLoss(nn.Module):
    """Transmission-Calibrated Boundary Loss for underwater object detection.

    Extends the Normalized Wasserstein Distance (NWD) framework (Wang et al. 2021)
    by introducing Beer-Lambert guided variance calibration for bounding box regression.

    Scientific motivation
    ---------------------
    Underwater light scattering blurs object boundaries, making precise bounding box
    localisation inherently uncertain.  Existing IoU-based losses assume hard boundaries,
    which is physically invalid in turbid water.

    TCB Loss models each bounding box as a 2D Gaussian N(mu, Sigma) where:

        Sigma = diag((sigma*w/2)**2, (sigma*h/2)**2)
        sigma = 1 + beta * a          (attenuation-calibrated scaling)

    The attenuation severity `a` is estimated from residual feature map activation
    after A²FM compensation, creating a self-consistent optimisation loop:

        A²FM estimates transmission t -> compensates features
            -> TCB reads residual activation -> softens boundary constraint
            -> gradient signal refines A²FM parameters

    The Gaussian Wasserstein distance (GWD) provides smooth, scale-invariant
    gradients even when predicted and target boxes have zero overlap — critical
    for small underwater objects (jellyfish, small fish).

    Progressive activation
    ----------------------
    - beta  = 0 at init  => sigma = 1  (standard NWD, no calibration)
    - alpha = 0 at init  => pure CIoU  (safe plug-and-play, cannot degrade baseline)
    Both parameters activate gradually during training.
    ...
    - Akkaynak & Treibitz, "Sea-Thru" (CVPR 2018).
    """

    def __init__(self, reg_max: int = 16):
        """Initialise TCBBoxLoss.

        Args:
            reg_max: regularisation maximum for Distribution Focal Loss.
        """
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.reg_max = reg_max

        # ---- Learnable parameters (progressive, init 0 => safe plug-and-play) ----
        # beta:  attenuation calibration strength
        #   sigma = 1 + beta * a,  beta=0 => sigma=1 => standard NWD variance
        self.beta = nn.Parameter(torch.zeros(1))
        # alpha_raw:  GWD weight in [CIoU, GWD] interpolation
        #   sigmoid(alpha_raw) with alpha_raw=-5 at init => sigmoid(-5) ≈ 0.007 ≈ 0
        self.alpha_raw = nn.Parameter(torch.tensor(-5.0))

        # C: normalisation constant for Wasserstein distance.
        # Initialised to a reasonable default for 640-pixel images;
        # will be updated via exponential moving average during training.
        self.register_buffer("C", torch.tensor(10.0))
    ...
    def forward(
        ...
        """Compute TCB box loss = (1-alpha)*CIoU + alpha*GWD + DFL.

        Args:
            pred_dist:     (B, N_anchors, 4*reg_max) predicted distribution
            pred_bboxes:   (B, N_anchors, 4)        predicted boxes in xyxy
            anchor_points: (N_anchors, 2)            anchor centre coords
            target_bboxes: (B, N_anchors, 4)         target boxes in xyxy (strided)
            target_scores: (B, N_anchors, N_cls)     target scores
            target_scores_sum: scalar                 max(Σ target_scores, 1)
            fg_mask:       (B, N_anchors)            foreground mask
            imgsz:         (2,)                       (h, w) of input image
            stride:        (N_anchors, 1)             stride tensor
            feats:         list[Tensor(B,C,H,W)]      detection feature maps

        Returns:
            (loss_box, loss_dfl) tuple.
        """
        # ---- Per-object weight (same as BboxLoss) ----
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)  # (N_fg, 1)

        # ---- 1. Standard CIoU loss ----
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
        loss_ciou = ((1.0 - iou) * weight).sum() / target_scores_sum

        # ---- 2. Gaussian Wasserstein Distance loss (attenuation-calibrated) ----
        if feats is not None and fg_mask.sum() > 0:
            loss_gwd = self._gwd_loss(
            ...
            loss_gwd = torch.tensor(0.0, device=loss_ciou.device)

        # ---- 3. Progressive CIoU-GWD interpolation ----
        alpha = self.alpha_raw.sigmoid()  # ≈ 0 at init, activates during training
        loss_box = (1.0 - alpha) * loss_ciou + alpha * loss_gwd

        # ---- 4. DFL loss (identical to BboxLoss) ----
        if self.dfl_loss:
            target_ltrb = bbox2dist(anchor_points, target_bboxes, self.dfl_loss.reg_max - 1)
            loss_dfl = (
                self.dfl_loss(
            ...
            loss_dfl = (
                F.l1_loss(
                    pred_dist_norm[fg_mask], target_ltrb[fg_mask], reduction="none"
                ).mean(-1, keepdim=True)
                * weight
            )
            loss_dfl = loss_dfl.sum() / target_scores_sum
    ...
    # ------------------------------------------------------------------
    #  Gaussian Wasserstein Distance (attenuation-calibrated)
    # ------------------------------------------------------------------
    ...
    def _gwd_loss(
        ...
        """Compute attenuation-calibrated Gaussian Wasserstein Distance loss.

        Steps:
            1. Estimate per-object attenuation severity a from feature maps
            2. Convert xyxy boxes -> (cx, cy, w, h) Gaussians
            3. Apply calibration: sigma = 1 + beta * a
            4. Compute Wasserstein distance W2^2
            5. Normalise: NWD = exp(-sqrt(W2^2) / C),  L = 1 - NWD
        """
        eps = 1e-7

        # ---- Step 1: Attenuation severity per foreground object ----
        a = self._attenuation_severity(fg_mask, anchor_points, stride, feats)  # (N_fg,)

        # ---- Step 2: Extract foreground boxes in xyxy -> Gaussian params ----
        p = pred_bboxes[fg_mask]   # (N_fg, 4)  xyxy
        g = target_bboxes[fg_mask]  # (N_fg, 4)  xyxy

        w_p, h_p = p[:, 2] - p[:, 0], p[:, 3] - p[:, 1]
        w_g, h_g = g[:, 2] - g[:, 0], g[:, 3] - g[:, 1]
        cx_p, cy_p = (p[:, 0] + p[:, 2]) * 0.5, (p[:, 1] + p[:, 3]) * 0.5
        cx_g, cy_g = (g[:, 0] + g[:, 2]) * 0.5, (g[:, 1] + g[:, 3]) * 0.5

        # ---- Step 3: Attenuation-calibrated variance scaling ----
        # sigma >= 1 (when a >= 0, beta >= 0 in practice)
        # beta starts at 0 -> sigma = 1 -> standard NWD, no calibration
        sigma = (1.0 + self.beta * a).clamp(min=0.1, max=10.0)  # (N_fg,)

        # ---- Step 4: Gaussian Wasserstein distance ----
        # For diagonal Sigma = diag((sigma*w/2)^2, (sigma*h/2)^2):
        #   W2^2 = ||mu_p - mu_g||^2 + ||Sigma_p^(1/2) - Sigma_g^(1/2)||_F^2
        #        = d_mu^2 + (sigma*w_p/2 - sigma*w_g/2)^2
        #                 + (sigma*h_p/2 - sigma*h_g/2)^2
        #        = d_mu^2 + sigma^2/4 * [(w_p - w_g)^2 + (h_p - h_g)^2]
        d_mu2 = (cx_p - cx_g).pow(2) + (cy_p - cy_g).pow(2)          # centre distance^2
        d_size2 = (w_p - w_g).pow(2) + (h_p - h_g).pow(2)             # size discrepancy^2
        w2 = d_mu2 + (sigma.pow(2) / 4.0) * d_size2                   # W_2^2
        w2 = w2.clamp(min=eps)                                        # numerical safety

        # ---- Step 5: Normalise to (0, 1] via exponential mapping ----
        sqrt_w2 = w2.sqrt()
        # Update normalisation constant via EMA (momentum 0.01)
        if self.training:
            with torch.no_grad():
                self.C.mul_(0.99).add_(sqrt_w2.detach().mean() * 0.01)
        nwd = torch.exp(-sqrt_w2 / self.C.clamp(min=eps))
        loss_gwd = ((1.0 - nwd).unsqueeze(-1) * weight).sum() / target_scores_sum

        return loss_gwd

    # ------------------------------------------------------------------
    #  Attenuation Severity Estimation
    # ------------------------------------------------------------------

    @staticmethod
    def _attenuation_severity(
        fg_mask: torch.Tensor,
        anchor_points: torch.Tensor,
        stride: torch.Tensor,
        feats: list[torch.Tensor],
    ) -> torch.Tensor:
        """Estimate per-object attenuation severity from feature map statistics.

        For each detection level, compute a spatial attenuation map from the
        post-A²FM feature map as:

            a(h, w) = 1 - sigmoid(mean_channel_activation(h, w))

        where low channel-mean activation indicates that A2FM could not fully
        recover the signal — a proxy for residual Beer-Lambert attenuation.

        Per-object severity is obtained by bilinear interpolation at each
        foreground anchor's grid position.

        Args:
            fg_mask:       (B, N_total)                foreground mask
            anchor_points: (N_total, 2)                grid coords [0.5 .. W-0.5]
            stride:        (N_total, 1)                per-anchor stride
            feats:         list[Tensor(B,C,H,W)]       P3, P4, P5 feature maps

        Returns:
            a: (N_fg,)  attenuation severity per foreground anchor
        """
        device = fg_mask.device
        batch_size = fg_mask.shape[0]
        total_anchors = anchor_points.shape[0]

        # Accumulate severity per anchor; we only need values at fg_mask positions
        a_full = torch.zeros(batch_size, total_anchors, device=device)

        anchor_start = 0
        for level_idx, feat in enumerate(feats):
            _, _, H, W = feat.shape
            n_anchors_level = H * W
            anchor_end = anchor_start + n_anchors_level

            # ---- Spatial attenuation map for this level ----
            # feat: (B, C, H, W) -> mean over channels -> (B, H, W)
            activation = feat.mean(dim=1)  # mean channel activation
            # Low activation => residual attenuation is high
            # 1 - sigmoid(x) maps: high x -> ~0 (clear), low x -> ~1 (attenuated)
            atten_map = 1.0 - activation.sigmoid()  # (B, H, W)

            # ---- Anchor grid positions for this level ----
            level_anchors = anchor_points[anchor_start:anchor_end]  # (H*W, 2)
            # anchor coords in [0.5, W-0.5] -> integer pixel indices
            x_idx = level_anchors[:, 0].long().clamp(0, W - 1)  # (H*W,)
            y_idx = level_anchors[:, 1].long().clamp(0, H - 1)  # (H*W,)

            # ---- Gather at anchor positions (vectorised over batch) ----
            # atten_map[b, y, x] -> (B, H*W)
            level_severity = atten_map[:, y_idx, x_idx]  # (B, H*W)
            a_full[:, anchor_start:anchor_end] = level_severity

            anchor_start = anchor_end

        # ---- Extract foreground anchors only ----
        a_fg = a_full[fg_mask]  # (N_fg,)
        return a_fg
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        Args:
            model: detection model (de-paralleled).
            tal_topk: top-k for task-aligned assigner.
            tal_topk2: secondary top-k (for certain dual-assignment heads).
            loss_type: "ciou" (standard) or "tcb" (Transmission-Calibrated Boundary).
        """
        ...
        if loss_type == "tcb":
            self.bbox_loss = TCBBoxLoss(m.reg_max).to(device)
        else:
