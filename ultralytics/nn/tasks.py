...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        if isinstance(self.args, dict):
            loss_type = self.args.get("loss_type", "ciou")
        else:
            loss_type = getattr(self.args, "loss_type", "ciou")
        if getattr(self, "end2end", False):
            return E2ELoss(self)
        return v8DetectionLoss(self, loss_type=loss_type)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = c1
            args = [c1]
        elif m is DCAConv_v2:
            c1 = ch[f] if isinstance(f, int) else ch[f[0]]
            c2 = c1
            args = [c1]
        elif m is DCAConv_v3:
            c1 = ch[f] if isinstance(f, int) else ch[f[0]]
            c2 = c1
            args = [c1]
        elif m is CGFusion:
            # f is a list of input indices; c1_list = per-source channels
            ...
            c_shallow = ch[f[1]]  # backbone skip
            c_deep = ch[f[0]]    # neck previous
            c2 = c_shallow + c_deep
